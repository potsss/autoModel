...
- “破坏者”在每一代都会对所有模型进行批判，而“控制单元”会周期性地**加大惩罚力度**（增加批判性分数的权重），从而提升“对抗压力”。
...
7.  **动态调整与循环 (`main.py`)**: 在每个`challenge_interval`（例如每5个世代），`控制单元`会自动增加“批判”分数在综合评估中的权重，从而加大“对抗压力”。然后系统进入下一代演化，重复第3步。
8.  **循环与收敛**：以上过程循环往复，每一代都会产出比上一代更优秀的建模策略。最终，系统在指定的演化代数完成后，输出综合表现最佳的“冠军模型”。
